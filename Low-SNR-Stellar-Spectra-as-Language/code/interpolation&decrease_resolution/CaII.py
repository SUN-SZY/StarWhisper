...
def process_spectrum(infile, outfile_prefix, snr):
    ...
    with fits.open(infile) as hdul:
        ...
        # 添加噪声
        noisy_flux_range = add_noise(flux_range, snr)

        # ✅ 新增：仅在首次运行时保存 wavelength_range
        if not WAVELENGTH_SAVED:
            np.savetxt(os.path.join(output_base, "wavelength.dat"), wavelength_range)
            print(f"已保存 wavelength.dat 到 {output_base}")
            WAVELENGTH_SAVED = True

        # 保存到 CSV（只保留 noisy flux）
        save_to_csv(noisy_flux_range, outfile_prefix + ".csv")
