...
# 与预训练数据集完全一致的列顺序（新增 augmentation_id）
OUTPUT_COLUMNS = [
    'spectrum_id', 'augmentation_id', 'pixel_idx',
...
def convert_lamost_to_pretrain_format(input_csv, catalog_path, output_csv, max_rows=None, chunksize=1_000_000):
    ...
    for chunk_idx, chunk in enumerate(pd.read_csv(input_csv, chunksize=chunksize)):
        ...
        for _, row in chunk.iterrows():
            obsid = str(row['obsid'])
            aug_id = str(row.get('augmentation_id', ""))
...
def _append_file_to_file(src_path: str, dst_path: str, header_already_written: bool) -> bool:
    """
    将 src_path 追加到 dst_path。
    - 若 header_already_written=True，则跳过 src 的首行表头；
    - 若为 False，则保留首行表头并写入。
    返回：下游应当把返回值当作“表头已写入”的标志（恒为 True，只要调用成功）。
    """
    if not os.path.exists(src_path) or os.path.getsize(src_path) == 0:
        return header_already_written
    with open(src_path, 'r', encoding='utf-8') as src, open(dst_path, 'a', encoding='utf-8') as dst:
        first = True
        for line in src:
            if header_already_written and first:
                first = False
                continue
            dst.write(line)
    return True
...
def shuffle_and_split_data(
    ...
    # 首先获取所有唯一的 spectrum_id（按 obsid 进行划分，增强样本共享同一obsid）
    print("   📊 统计增强样本数量（obsid+augmentation_id）...")
    unique_sample_ids = set()
    for chunk in pd.read_csv(
        input_csv,
        usecols=['spectrum_id', 'augmentation_id'],
        dtype={'spectrum_id': str, 'augmentation_id': str},
        chunksize=1_000_000,
    ):
        sample_ids = (chunk['spectrum_id'].astype(str) + '_' + chunk['augmentation_id'].fillna('').astype(str)).unique()
        unique_sample_ids.update(sample_ids)

    sample_ids = list(unique_sample_ids)
    print(f"   总增强样本数: {len(sample_ids):,}")

    # 按增强样本进行9:1划分（允许同obsid的不同augmentation落入不同集合）
    ...
    print(f"   训练集增强样本数: {len(train_ids):,}")
    print(f"   验证集增强样本数: {len(val_ids):,}")
    ...
    for chunk in tqdm(
        ...
        # 计算增强样本ID（obsid + '_' + augmentation_id）
        chunk['sample_id'] = chunk['spectrum_id'].astype(str) + '_' + chunk['augmentation_id'].fillna('').astype(str)

        # 仅保留在 train/val 中的行
        chunk = chunk[chunk['sample_id'].isin(train_ids_set | val_ids_set)]
        if chunk.empty:
            continue
    ...
    # 分片缓冲，减少磁盘写次数
        ...
        grouped = chunk.groupby(['spectrum_id', 'augmentation_id'], sort=False)
        for (sid, aug), g in grouped:
            sid_str = str(sid)
            aug_str = str(aug) if aug is not None else ''
            key = f"{sid_str}_{aug_str}"
            df = g
            if key in carry:
                df = pd.concat([carry[key], df], ignore_index=True)
                del carry[key]
            ...
            while len(df) >= expected_pixels:
                ...
                shard_idx = _stable_shard_index(key, num_shards, random_state)
                if key in train_ids_set:
    ...
    for idx in shard_order:
        ...
        if os.path.exists(t_shard):
            train_header_written = _append_file_to_file(t_shard, train_path, header_already_written=train_header_written)
        if os.path.exists(v_shard):
            val_header_written = _append_file_to_file(v_shard, val_path, header_already_written=val_header_written)
    ...
    _ = _append_file_to_file(train_path, full_path, header_already_written=False)
    _ = _append_file_to_file(val_path, full_path, header_already_written=True)
